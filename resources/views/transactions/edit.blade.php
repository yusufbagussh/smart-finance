            ...
            <div class="p-6">
                ...
                <form method="POST" action="{{ route('transactions.update', $transaction) }}" x-data="transactionForm()">
                    ...
                    <div class="mb-6" x-show="type === 'expense'" x-transition>
                        <label for="liability_id" class="block text-sm font-medium text-gray-700 mb-2">Link to Debt
                            (Optional)</label>
                        <select name="liability_id" id="liability_id"
                            ...
                            @foreach ($liabilities as $liability)
                                <option value="{{ $liability->id }}"
                                    ...
                                    {{ $liability->name }} (Balance: Rp
                                    {{ number_format($liability->current_balance, 0, ',', '.') }})
                                </option>
                            @endforeach
                        </select>
                        @error('liability_id')
