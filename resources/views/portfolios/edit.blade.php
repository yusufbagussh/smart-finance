        ...
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 sm:p-8">
                    {{-- Form mengarah ke .update dan menggunakan method PUT --}}
                    <form method="POST" action="{{ route('portfolios.update', $portfolio) }}" class="space-y-6">
                        ...
                        <div>
                            ...
                            <input type="text" name="name" id="name" {{-- Mengisi value dari $portfolio --}}
                        ...
                        <div>
                            ...
                            <textarea name="description" id="description" rows="3"
                                class="mt-1 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md dark:bg-gray-700 dark:border-gray-600 dark:text-gray-200 dark:focus:ring-indigo-500 dark:focus:border-indigo-500"
                                placeholder="e.g., Long-term investments in stocks and bonds.">{{-- Mengisi value dari $portfolio --}}{{ old('description', $portfolio->description) }}</textarea>
                            @error('description')
                                <p class="mt-2 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="pt-6 border-t border-gray-200 dark:border-gray-700 space-y-6">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    ...
                                    <select name="risk_profile" id="risk_profile"
                                        ...
                                        <option value="conservative"
                                            {{ old('risk_profile', $portfolio->risk_profile) == 'conservative' ? 'selected' : '' }}>
                                            Konservatif (Stabil, pertumbuhan lambat)</option>
                                        <option value="moderate"
                                            {{ old('risk_profile', $portfolio->risk_profile) == 'moderate' ? 'selected' : '' }}>
                                            Moderat (Seimbang antara risiko & hasil)</option>
                                        <option value="aggressive"
                                            {{ old('risk_profile', $portfolio->risk_profile) == 'aggressive' ? 'selected' : '' }}>
                                            Agresif (Siap ambil risiko tinggi)</option>
                                    </select>
                                    @error('risk_profile')
                                        <p class="mt-2 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                                    @enderror
                                ...
                                <div>
                                    ...
                                    <input type="number" name="time_horizon" id="time_horizon"
                                        value="{{ old('time_horizon', $portfolio->time_horizon) }}"
                            ...
                            <div>
                                ...
                                <textarea name="future_plans" id="future_plans" rows="3"
                                    ...
                                    placeholder="Misal: Fokus menambah Reksadana, mulai masuk saham, dll.">{{ old('future_plans', $portfolio->future_plans) }}</textarea>
                                @error('future_plans')
                                    <p class="mt-2 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div>
                                ...
                                <textarea name="risk_tolerance_notes" id="risk_tolerance_notes" rows="3"
                                    ...
                                    placeholder="Misal: Saya tidak nyaman jika portofolio turun lebih dari 10% setahun.">{{ old('risk_tolerance_notes', $portfolio->risk_tolerance_notes) }}</textarea>
